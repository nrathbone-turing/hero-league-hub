...
from backend.app.models import Entrant, Match, Hero, User
from backend.app.extensions import db
from sqlalchemy import select


def test_user_registers_for_event(client, create_event, create_user, create_hero, auth_header):
    event = create_event()
    user = create_user()
    hero = create_hero()
...
def test_prevent_duplicate_registration(client, create_event, create_user, create_hero, auth_header):
    event = create_event()
    user = create_user()
    hero = create_hero()
...
def test_create_entrant_admin(client, create_event, auth_header):
    event = create_event()
...
def test_get_entrants_filter_by_event_and_user(client, create_event, create_user, create_hero, auth_header):
    event = create_event()
    user = create_user()
    hero = create_hero()
...
def test_delete_entrant_hard_and_soft(client, seed_event_with_entrants, session, auth_header):
